...
Each of next t lines contains two space separated integers: d, the square of the radius of the city and k, the maximum number of police stations the headquarters is willing to build.
...
bool Police(int n,int k){
	unordered_map<int,int>::iterator it;
	it=numbers.find(n);
	if(it!=numbers.end())
		{return k>=numbers[n];}
 	else
		{int r=sqrt(n);int tmp=0;
		 for(int i=1;i<r;i++)
			{int j=sqrt(n-i*i);
			 if((j*j+i*i)==n){tmp++;}
			}
